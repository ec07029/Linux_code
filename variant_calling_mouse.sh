...
# Split reads
ls -d Sample*[0-9] | parallel 'java -jar /data/leslie/chuk/jartools/GenomeAnalysisTK-3.8-1-0-gf15c1c3ef/GenomeAnalysisTK.jar -T SplitNCigarReads -R /data/leslie/chuk/index_Karen/GRCm38_GencodeM15/GRCm38.primary_assembly.genome.fa -I {}.dedupped.bam -o {}.split.bam -rf ReassignOneMappingQuality -RMQF 255 -RMQT 60 -U ALLOW_N_CIGAR_READS'

# Call SNPs & annotate
ls -d Sample*[0-9] | parallel 'java -jar /data/leslie/chuk/jartools/GenomeAnalysisTK-3.8-1-0-gf15c1c3ef/GenomeAnalysisTK.jar -T HaplotypeCaller -R /data/leslie/chuk/index_Karen/GRCm38_GencodeM15/GRCm38.primary_assembly.genome.fa -I {}.split.bam -dontUseSoftClippedBases -stand_call_conf 10.0 -o {}.HaplotypesR.vcf --dbsnp /data/leslie/chuk/index_Karen/dbsnp_mouse/dbsnp_151_mouse.sorted.vcf'
ls -d Sample*[0-9] | parallel 'java -jar /data/leslie/chuk/jartools/GenomeAnalysisTK-3.8-1-0-gf15c1c3ef/GenomeAnalysisTK.jar -T VariantFiltration -R /data/leslie/chuk/index_Karen/GRCm38_GencodeM15/GRCm38.primary_assembly.genome.fa -V {}.HaplotypesR.vcf -window 35 -cluster 3 -filterName FS -filter "FS > 30.0" -filterName QD -filter "QD < 2.0" -o {}.FinalR.vcf'
